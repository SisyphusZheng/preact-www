...
useResult(function (result) {
  var clicks = [];
  var items = result.output.querySelectorAll('li');
  items.forEach(function (item) {
    ...
    if (cb) {
      cb.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var lis = result.output.querySelectorAll('li');
        var idx = Array.prototype.indexOf.call(lis, item);
        clicks.push(idx);
        if (idx === 0 && clicks.length === 1) {
          var spy = document.createElement('span');
          spy.id = 'spy';
          spy.style.cssText = 'position:fixed;left:-1000px;';
          result.output.appendChild(spy);
          setTimeout(function(){
            var lis2 = result.output.querySelectorAll('li');
            var passed = lis2.length === 2 && lis2[0].textContent.trim().indexOf('买牛奶') === 0;
            if (passed) spy.className = 'pass';
            result.redraw();
            if (passed) solutionCtx.setSolved(true);
          }, 1);
        }
      });
    }
...
function App() {
  ...
  function completeTodo(completedTodo) {
    // 从列表中筛选掉已完成的todo
    const newTodos = todos.filter(todo => todo !== completedTodo);
    setTodos(newTodos);
  }
    ...
    <div>
      ...
      <ul>
        {todos.map(todo => (
          <li>
            <label>
              <input type="checkbox" onClick={() => completeTodo(todo)} />
              {todo.text}
            </label>
          </li>
        ))}
