...
在这一步中，你仍然使用任何 UI 库构建组件树，如 Vue、React、Preact、Solid 等。然后将组件树压平，只保留静态和可立即计算的数据。在这种情况下，不运行副作用和状态管理相关的代码。
...
简单来说，就是将交互性绑定到具有_现有_状态/效果/事件的 DOM 元素上。

这些_现有_状态/效果/事件可能是从服务器发送的，但如果使用的组件可以处理自己的逻辑，并且逻辑很好地包含在组件中，你只需要在 DOM 上挂载组件并进行必要的绑定。
...
我上面解释的称为部分水合，因为你不是水合整个应用程序，而只是水合其中的特定部分。
...
查看导入，我们有与[开始](#开始)部分中提到的相同导入，没有太大变化。
...
这个名称很自解释，因为我们在这里不做任何关于什么是孤岛、什么不是孤岛的区分，你可以像平常一样将所有组件放在这里。
